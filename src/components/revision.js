import React, { useState } from 'react';
import { jsPDF } from 'jspdf';
import eventsData from '../data/dates.json'
import '../styles/revision.scss'


const imgpdf = 'data:image/webp;base64,'

const years = Array.from(new Set(eventsData.map(event => event.annee))).sort((a, b) => a - b);
const yearOptions = years.map(year => ({ value: year, label: year.toString() }));

const PdfGenerator = () => {
    const [startYear, setStartYear] = useState('');
    const [endYear, setEndYear] = useState('');
    const [isCollapsed, setIsCollapsed] = useState(true);

    const handleGeneratePDF = () => {
        const start = parseInt(startYear, 10);
        const end = parseInt(endYear, 10);

        if (isNaN(start) || isNaN(end) || start < -100 || end > 2024) {
            alert('Veuillez entrer des années valides entre -100 et 2024.');
            return;
        }

        if (start > end) {
            alert('L\'année de début ne peut pas être après l\'année de fin.');
            return;
        }

        const filteredEvents = eventsData.filter(event =>
            event.annee >= start && event.annee <= end
        );

        if (filteredEvents.length === 0) {
            alert('Aucun événement trouvé pour la plage d\'années sélectionnée.');
            return;
        }

        const doc = new jsPDF();
        const pageWidth = doc.internal.pageSize.width;
        const pageHeight = doc.internal.pageSize.height;

        // Définir la couleur du texte
        const textColor = [12, 26, 43];
        doc.setTextColor(textColor[0], textColor[1], textColor[2]);

        // Fonction pour ajouter l'en-tête à chaque page
        const addHeader = () => {
            doc.setFontSize(20);
            doc.setFont('courier', 'normal');
            doc.text('Liste des Événements', pageWidth / 2, 20, { align: 'center' });
            doc.setDrawColor(textColor[0], textColor[1], textColor[2]);
            doc.setLineWidth(0.5); // Épaisseur de la ligne
            doc.line(10, 25, pageWidth - 10, 25); // Coordonnées de la ligne
        };

        // Ajouter la première page
        addHeader();

        const imageX = 90; // Position X de l'image
        const imageY = 30; // Position Y de l'image
        const imageWidth = 25; // Largeur de l'image
        const imageHeight = 25; // Hauteur de l'image

        // Ajouter l'image
        doc.addImage(imgpdf, 'WEBP', imageX, imageY, imageWidth, imageHeight);

        let y = imageY + imageHeight + 10; // Ajuster Y pour le texte après l'image

        // Ajouter les événements avec l'année et des retours à la ligne
        filteredEvents.forEach(event => {
            const yearText = `${event.annee} :`; // Année avec deux-points

            event.evenement.forEach((text, index) => {
                if (y > pageHeight - 40) { // Nouvelle page si nécessaire
                    doc.addPage();
                    addHeader(); // Réajouter l'en-tête
                    y = 35; // Réinitialiser Y
                }

                doc.setFont('helvetica', 'normal'); // Changer la police
                const lines = doc.splitTextToSize(`${yearText} ${text}`, pageWidth - 30);

                lines.forEach((line, i) => {
                    doc.text(line, 20, y); // Décalage de X pour le texte
                    y += 10; // Ajuster Y pour le prochain texte
                });

                y += 10; // Ajouter un espace après chaque événement
            });
        });

        // Pied de page
        doc.setFontSize(10);
        doc.text('Généré avec Quelle-Date.com', pageWidth / 2, pageHeight - 10, { align: 'center' });

        // Sauvegarder le PDF
        doc.save('fiche_revision.pdf');
    };

    return (
        <div className="revisionContainer">
            <div
                className="collapseHeader"
                onClick={() => setIsCollapsed(!isCollapsed)}
            >
                {isCollapsed ? 'Créer votre fiche de révision' : 'Replier'}
            </div>
            <div className={`collapseContent ${!isCollapsed ? 'expanded' : ''}`}>
                <h2>Ma fiche de révision</h2>
                <div>
                    <label htmlFor="startYear">De l'année :</label>
                    <input
                        id="startYear"
                        type="number"
                        min="-100"
                        max="2024"
                        value={startYear}
                        onChange={e => setStartYear(e.target.value)}
                        placeholder="Entrez l'année de début"
                    />
                </div>
                <div>
                    <label htmlFor="endYear">À l'année :</label>
                    <input
                        id="endYear"
                        type="number"
                        min="-100"
                        max="2024"
                        value={endYear}
                        onChange={e => setEndYear(e.target.value)}
                        placeholder="Entrez l'année de fin"

                    />
                </div>
                <button className="button" onClick={handleGeneratePDF}>Générer le PDF</button>
            </div>
        </div>
    );
};

export default PdfGenerator;